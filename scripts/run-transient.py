...
def wind_speed_schedule(hour):
    return -0.2892 * hour + 9.486

def ambient_temperature_schedule(hour):
    return 0.5*(1.0-math.tanh(hour-21.5))*math.cos(0.385*hour)/0.2 - 0.4870*hour + 16.84

def ambient_pressure_schedule(hour):
    return 450.0 * math.tanh(hour-7.0) + 9.706 * hour + 97760.0

def wind_pressure_profile(angle):
    C00 = 6.00000e-01
    # C01 = 0.00000e+00
    C02 = -1.23300e-04
    C03 = -2.20300e-06
...
def wind_direction_schedule(hour):
    return afn.Angle(150*math.cos((hour/12.0)**4) + 6.739*hour + 75, "degrees")

this_dir = os.path.abspath(os.path.dirname(__file__))
json_file = os.path.join(this_dir, '..', 'examples', 'law-office.json')
